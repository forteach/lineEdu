...
public interface CatalogueRepository extends JpaRepository<Catalogue, String>, JpaSpecificationExecutor<CatalogueRepository> {
    ...
    @Query("select chapterId as chapterId, chapterName as chapterName, chapterParentId as chapterParentId, publish as publish, sort as sort, chapterLevel as chapterLevel " +
            "from Catalogue where  courseId = ?1 order by chapterLevel asc, sort asc")
...
//    List<ICourseChapterDto> findCourseId(@Param("isValidated") String isValidated, @Param("courseId") String courseId);
    ...
    @Transactional(readOnly = true)
    List<Catalogue> findByCourseIdAndAndChapterParentId(String courseId, String chapterParentId);
